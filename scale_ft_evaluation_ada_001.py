...
import time
fails = [
    " will print 0-1 if the input is good, and 3-5 if",
    " would be considered equivalent if they had the same performance.",
    " are safe.",
    " would be considered equivalent if they had the same return type and memory layout.",
    " will return an error if the number you are trying to return is greater than one",
    " would be equivalent if a and b were both 1."
]
MAX_RETRIES = 1
            ...
            while retries < MAX_RETRIES:
                try:
                    ...
                    if 'no' in response_content or 'are not equivalent' in response_content:
                        prediction = 0
                    elif 'yes' in response_content or 'are equivalent' in response_content or 'produce the same result' in response_content or 'are the same' in response_content or 'have the same behavior' in response_content or 'are same' in response_content or 'have the same effect' in response_content or "would be considered the same" in response_content or "have the same output" in response_content or "have the same return value but different instructions" in response_content:
                        prediction = 1
                    elif response_content in fails:
                      prediction = -1
                    else:
                        retries += 1
                        print(response)
...
preds = adjust_predictions(preds, labels)
